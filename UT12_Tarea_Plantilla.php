...
<!--
		...
		Al pulsar el botón, se ejecutará alguna de las siguientes acciones:
				...
				*	Si no hay nombre y no hay telefono se saca todo por pantalla
...
<html>
		...
		<body>
				<?php
						...
						//poner los if para ver que combinacion de condiciones corresponde con cada operacion 

						if(!empty($_POST['nom'])){//$nomhay nombre

							if (!empty($tfno)){//hay telefono
								$encontrado=findPos($array,$_POST('tfno'));
								

								if (!$encontrado){//TELEFONO NO EXISTE....INSERT
									echo "Insert<br>\n";
									$array[$count++]=$_POST['tfno'];
									$array[$count++]=$_POST['nom'];
								}else{//telefono existe.... UPDATE
									echo "...UPDATE<br>";
							...
							}else{//no hay telefono... listados de telefonos de ese nombre
									echo "..listado TO DO";
							}
						
						}else{//no hay nombre

							if (!empty($_POST['tfno'])){//TELEFONO NO EXISTE....GETALL

								
								echo "<br>";
								echo "... GETALL<br>\n";
								foreach ($array as $v) echo"$v..;<br>\n";



							}else{//telefono existe.... DELETE
								echo "....DELETE<br>";
